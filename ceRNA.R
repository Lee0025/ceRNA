...
## prognostic model development
s$Score <- 0.2625*s$OPN3 + 0.2724*s$DAAM2 - 0.0794*s$HENMT1 + 0.0202*s$CAVIN3
s <- s[order(s$Score),]
s$Rank <- 1:nrow(s)
s$Risk <- ifelse(s$Score > median(s$Score),'High','Low')
library(ggplot2)
p8 <- ggplot(s,aes(x = Rank,y = Score,color=Risk))+geom_point()+
      scale_color_manual(values = c('green','red'))
p9 <- ggplot(s,aes(x = Score,y=days/365,color=statu))+geom_point()+
      scale_color_manual(values = c('green','red'))
